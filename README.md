...
# 开发历程
本程序最初于19年底，由最初的Python脚本验证瓦片下载可行性，之后使用NodeJS重构为本地服务，再然后萌生为作为独立桌面端的想法，使用Electron再次重构，后来遇到性能问题，全部推翻用Java Swing进行重写，最终到了目前Swing+SpringBoot+JCEF+Angular的模式，包含前后端，用到了不少技术去实现。

于是本项目从最初的以实现功能为主，演变到了更侧重于作者对于各种技术的学习与实际运用。

为什么选择Java做界面，因为考虑到做GIS开发，大部分也是做Web开发的人员。选择Java，就能更加贴合各位所掌握的技术。至于QT，或者C#，这两个桌面端首选技术栈，我都能做，但是这样就没有多少人愿意研究该项目的代码了。同时选择Java，也能通过Spring的生态来提高开发效率，比如mybatis+sqlite做行政区边界坐标存储与查询，以及okhttp代理与下载，再就是本地瓦片预览，以及程序与内置地图WebView的交互更加方便，等等。综上所述就可以知道本项目的技术栈，看似奇葩，其实也是考虑周全并合理。至于为什么要从Electron重构到Java+JCEF，因为多线程下载和大量的图片处理，JS单线程很难满足该需求。在今年以前，本项目里面的WebView一开始是靠第三方的JxBrowser，后来靠自己编译JCEF。今年JetBrains提供了内置JCEF的JDK，对本项目的开发来说也是一件好事。

开发过程中其实遇到了不少困难，作者也尽力去解决，同时也会参考各位的建议添加新功能。各位的点赞支持就是作者最大的开发动力。
